...
var DemoList = class extends import_coc.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "demo_list";
    this.description = "CocList for coc-helper";
  ...
  async loadItems(context) {
    return [
      {
        label: "coc-helper list item 1",
        data: { name: "list item 1" }
      },
      {
        label: "coc-helper list item 2",
...
async function activate(context) {
  import_coc2.window.showMessage(`coc-helper works!`);
  context.subscriptions.push(import_coc2.commands.registerCommand("coc-helper.Command", async () => {
    import_coc2.window.showMessage(`coc-helper Commands works!`);
  }), import_coc2.listManager.registerList(new DemoList(import_coc2.workspace.nvim)), import_coc2.sources.createSource({
    name: "coc-helper completion source",
    doComplete: async () => {
      const items = await getCompletionItems();
      return items;
    }
  }), import_coc2.workspace.registerKeymap(["n"], "helper-keymap", async () => {
